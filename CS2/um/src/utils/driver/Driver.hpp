...
namespace driver {
	...
	inline bool attach_to_process(HANDLE driver, const DWORD pid) {
		Request r;
	...
	template <class T>
	inline T read_memory(HANDLE driver, const std::uintptr_t addr) {
		T temp = {};
	...
	template <class T>
	inline void write_memory(HANDLE driver, const std::uintptr_t addr, const T& value) {
		Request r;
		r.target = reinterpret_cast<PVOID>(addr);
